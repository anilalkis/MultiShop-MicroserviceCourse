...
namespace MultiShop.WebUI.Areas.Admin.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> CreateCategory(CreateCategoryDto createCategoryDto)
        {
            ...
            var response = await client.PostAsync("https://localhost:7270/api/Categories", stringContent);
            if(response.IsSuccessStatusCode) 
            {
        ...
        public async Task<IActionResult> UpdateCategory(string id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync("https://localhost:7270/api/Categories/"+id);
            if(response.IsSuccessStatusCode)
            {
                var jsonData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateCategory(UpdateCategoryDto updateCategoryDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var response = await client.PutAsync("https://localhost:7270/api/Categories/", stringContent);
            if(response.IsSuccessStatusCode) 
            {
                return RedirectToAction("Index", "Category", new { area = "Admin" });
            }
